...
def handoff_to(state: ChatState, specialist_mode: str) -> dict:
    ...
    action = registry.get("enrich").plan_action(
        ctx.user_id, contract, ctx.now, ctx.tz, ctx.locale)
