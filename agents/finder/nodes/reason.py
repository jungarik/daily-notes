"""Reason node: one LLM planning step for the finder graph.

Decides the next move — call a read tool, or answer. Once the read-tool step
budget is spent it makes a final tool-free call, so the model must answer
instead of looping. The only public entry point is `run`.
"""

import json
import logging

import config
from agents.finder.state import FinderState
from agents.runtime import model_gateway
from tools.conversation import READ_TOOL_SPECS

logger = logging.getLogger(__name__)

_UNAVAILABLE_REPLY = (
    "I couldn't reach the AI provider right now. Please try again in a moment."
)


def _create_assistant_message(message) -> dict:
    data = {
        "role": "assistant",
        "content": message.content,
    }

    if message.tool_calls:
        data["tool_calls"] = [{
            "id": call.id,
            "type": "function",
            "function": {
                "name": call.function.name,
                "arguments": call.function.arguments,
            },
        } for call in message.tool_calls]

    return data


def _extract_first_tool(message) -> dict | None:
    if not message.tool_calls:
        return None

    tool_call = message.tool_calls[0]

    try:
        args = json.loads(tool_call.function.arguments or "{}")
    except Exception:
        args = {}

    return {
        "id": tool_call.id,
        "name": tool_call.function.name,
        "args": args,
    }


def _unavailable(state: FinderState, kind: str) -> dict:
    return {
        "messages": [
            *state.get("messages", []),
            {"role": "assistant", "content": _UNAVAILABLE_REPLY},
        ],
        "steps": state.get("steps", 0) + 1,
        "tool_call": None,
        "reply": _UNAVAILABLE_REPLY,
        "trace": {**(state.get("trace") or {}), "model_error": kind},
    }


def run(state: FinderState) -> dict:
    messages = state.get("messages") or []
    use_tools = state.get("steps", 0) < config.AGENT_MAX_STEPS

    model_request = {
        "model": config.AGENT_MODEL,
        "messages": messages,
        "temperature": 0.2,
    }

    if use_tools:
        model_request.update(
            tools=READ_TOOL_SPECS,
            tool_choice="auto",
            parallel_tool_calls=False,
        )

    try:
        model_response = model_gateway.chat_completion(**model_request).choices[0].message
    except model_gateway.ModelGatewayError as exc:
        logger.warning("Finder model call failed: %s", exc.kind)

        return _unavailable(state, exc.kind)

    tool_call = _extract_first_tool(model_response) if use_tools else None
    state_update = {
        "messages": [*messages, _create_assistant_message(model_response)],
        "steps": state.get("steps", 0) + 1,
        "tool_call": tool_call,
    }

    if tool_call is None:
        state_update.update(reply=model_response.content or "")

    return state_update
