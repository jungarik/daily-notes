"""Case 3: the model that picks the next agent.

The gateway is stubbed, so this covers what the selector promises the router —
that it never raises, and that it never returns a name the router did not offer
...
class ChooseNameTests(unittest.TestCase):
    ...
    def test_a_named_candidate_is_chosen(self):
        self.assertEqual(
            "enrich",
            model_selector.choose_name('{"agent": "enrich"}', CANDIDATES))

    def test_an_explicit_null_declines(self):
        self.assertIsNone(model_selector.choose_name('{"agent": null}', CANDIDATES))
    ...
    def test_an_agent_that_was_not_offered_is_refused(self):
        ...
        self.assertIsNone(
            model_selector.choose_name('{"agent": "responder"}', CANDIDATES))
        self.assertIsNone(
            model_selector.choose_name('{"agent": "nonesuch"}', CANDIDATES))

    def test_prose_instead_of_json_declines(self):
        self.assertIsNone(
            model_selector.choose_name("I think enrich should go next!", CANDIDATES))

    def test_a_missing_key_declines(self):
        self.assertIsNone(model_selector.choose_name('{"choice": "enrich"}', CANDIDATES))

    def test_a_json_scalar_declines(self):
        """`json.loads("4")` parses fine and then has no `.get`."""
        self.assertIsNone(model_selector.choose_name("4", CANDIDATES))
        self.assertIsNone(model_selector.choose_name('"enrich"', CANDIDATES))

    def test_an_empty_answer_declines(self):
        self.assertIsNone(model_selector.choose_name("", CANDIDATES))
...
class PromptTests(unittest.TestCase):
    ...
    def test_the_prompt_lists_only_the_candidates_it_was_given(self):
        model_selector.select_agent_name(CANDIDATES, "save this", ())
    ...
    def test_the_prompt_carries_the_record_not_agent_prose(self):
        ...
        model_selector.select_agent_name(CANDIDATES, "save this", history)
...
class FailureTests(unittest.TestCase):
    ...
    def test_a_gateway_error_declines_rather_than_raising(self):
        ...
        self.assertIsNone(model_selector.select_agent_name(CANDIDATES, "go", ()))
...
class RouterIntegrationTests(unittest.TestCase):
    ...
    def test_it_matches_the_signature_the_router_calls(self):
        ...
        router = Router(registry, select_model=model_selector.select_agent_name)
