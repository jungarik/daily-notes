"""The enricher agent's side of the loop contract.
...
def _install_stubs():
    """Stand in for the modules `agents.enricher.agent` imports at module level."""
    planned = {"action": None}
    ...
    graph = types.ModuleType("agents.enricher.graph")
    graph.ACTION_PLAN_GRAPH = types.SimpleNamespace(
        invoke=lambda state: {"action": planned["action"]})

    prompts = types.ModuleType("agents.enricher.prompts")
    ...
    tools = types.ModuleType("tools.enricher")
    tools.TOOLS = {
    ...
    sys.modules["agents.enricher.graph"] = graph
    sys.modules["agents.enricher.prompts"] = prompts
    sys.modules["tools.enricher"] = tools
...
from agents.enricher import agent  # noqa: E402
