...
from agents.runtime.handoff_dispatch import HandoffDispatch, pending
...
def build_broker(specialist, ledger=None):
    ...
    broker = HandoffDispatch(registry, ledger or SimpleNamespace())
...
class HandoffBrokerTests(unittest.TestCase):
    ...
    def test_plan_without_an_action_is_not_planned(self):
        ...
        plan = handoff_dispatch.plan(broker.route("perform_action"), broker.registry, [], {"instruction": "do a thing"}, [], ctx())
    ...
    def test_stage_carries_the_agent_and_contract(self):
        specialist = SimpleNamespace(plan_action=Mock(return_value=ACTION))
        broker = build_broker(specialist)
        plan = handoff_dispatch.plan(broker.route("perform_action"), broker.registry, [], {"instruction": "move it"}, [], ctx())
        ...
        self.assertEqual(staged["handoff"], plan.request)
    ...
    def test_execute_runs_the_action_through_the_ledger_once(self):
        ...
        result = handoff_dispatch.execute(pending, broker.registry, broker.ledger, ctx())
    ...
    def test_execute_applies_a_selection_to_a_link_action(self):
        ...
        handoff_dispatch.execute(pending, broker.registry, broker.ledger, ctx(), selection=["9", 9, "nope", 10])
